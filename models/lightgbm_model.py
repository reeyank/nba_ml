...
Trains separate LightGBM models for points, rebounds, and assists prediction
"""
...
class LightGBMTrainer:
    """Trains LightGBM models for player stat prediction"""
    ...
    def train_model(self, X_train, y_train, X_val, y_val, target: str):
        """Train LightGBM model for a specific target"""
        print(f"\n{'='*60}")
        ...
        print(f"  Best score: {model.best_score['valid_0']['l1']:.4f}")
    ...
    def save_models(self, metrics: dict):
        ...
        version = f"lightgbm_v1.0.0_{datetime.now().strftime('%Y%m%d_%H%M%S')}"
        ...
        with open(model_type_path, 'w') as f:
            f.write("lightgbm")
        print(f"✓ Saved model type indicator")

        # Save minimal metadata to database (optional)
        try:
            metadata = {
                ...
                'is_active': 0  # Don't auto-activate, keep XGBoost as default
            }
